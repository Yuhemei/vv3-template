...
Guacamole.Keyboard = function Keyboard(element) {
  ...
  var KeydownEvent = function KeydownEvent(orig) {
    ...
    this.keysym =
      keysym_from_key_identifier(this.key, this.location) || keysym_from_keycode(this.keyCode, this.location);
  ...
  var KeyupEvent = function KeyupEvent(orig) {
    ...
    this.keysym =
      keysym_from_keycode(this.keyCode, this.location) || keysym_from_key_identifier(this.key, this.location);
...
Guacamole.SessionRecording = function SessionRecording(tunnel) {
  ...
  var seekToFrame = function seekToFrame(index, callback, delay) {
    ...
    seekTimeout = window.setTimeout(function continueSeek() {
      ...
      if (currentFrame !== index) seekToFrame(index, callback, Math.max(delay - (new Date().getTime() - startTime), 0));
...
Guacamole.SessionRecording._Frame.Instruction = function Instruction(opcode, args) {
  ...
  this.getSize = function getSize() {
    ...
    for (var i = 0; i < instruction.args.length; i++) size += instruction.args[i].length;
...
Guacamole.HTTPTunnel = function (tunnelURL, crossDomain, extraTunnelHeaders) {
  ...
  function handleHTTPTunnelError(xmlhttprequest) {
    ...
    // code provided by the browser
      ...
      close_tunnel(
        new Guacamole.Status(Guacamole.Status.Code.fromHTTPCode(xmlhttprequest.status), xmlhttprequest.statusText),
      );
...
Guacamole.WebSocketTunnel = function (tunnelURL) {
  ...
  this.connect = function (data) {
    ...
    socket.onopen = function (event) {
      ...
      // Ping tunnel endpoint regularly to test connection stability
      pingInterval = setInterval(function sendPing() {
        tunnel.sendMessage(Guacamole.Tunnel.INTERNAL_DATA_OPCODE, 'ping', new Date().getTime());
      }, PING_FREQUENCY);
    };
      ...
      do {
        ...
        if (terminator === ';') {
          ...
          if (tunnel.uuid === null) {
            ...
            if (opcode === Guacamole.Tunnel.INTERNAL_DATA_OPCODE) tunnel.setUUID(elements[0]);
...
Guacamole.StaticHTTPTunnel = function StaticHTTPTunnel(url, crossDomain, extraTunnelHeaders) {
  ...
  this.connect = function connect(data) {
    ...
    xhr.onerror = function httpError() {
      ...
      if (tunnel.onerror)
        tunnel.onerror(new Guacamole.Status(Guacamole.Status.Code.fromHTTPCode(xhr.status), xhr.statusText));
